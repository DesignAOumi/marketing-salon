...
> **現在の状況**: 🎉 **MVP 到達（Phase 3 完了）** — Docker でセルフホスト → 顧客/施術/売上/来店を蓄積 → 来店サイクル & 分析 → M6 で再来店候補を自動抽出しワンクリックで予約案・連絡文生成 → 連携なし（完全オフライン）アドバイスまでが、外部送信ゼロ・データ非格納で動作します。次は Phase 4（M5 GCal 連携・連携ありAI）。
